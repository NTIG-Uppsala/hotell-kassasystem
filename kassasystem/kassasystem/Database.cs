    ...
    public class Database
    {
        ...
        public List<Room> GetAvailableRooms(int epochStartDate, int epochEndDate)
        {
            ...
            for (int i = bookings.Count -1; i >= 0; i--) 
            {
                ...
                bool overlap = epochStartDate < intEndDate && intStartDate < epochEndDate;
                if (overlap)
                {
        ...
        public void CreateNewBooking(Int64 roomID, string GuestFirstName, string GuestLastName, int checkinDate, int checkoutDate, decimal totalPrice)
        {
            ...
            var newBookingID = QueryInsertExecutor($"INSERT INTO bookings (guestID, paymentID, dateFrom, dateTo, roomCount, isBreakfastIncluded, isRemoved) VALUES ('{newGuestID}', '{newPaymentID}', '{checkinDate}', '{checkoutDate}', '1', '1', '0') ");
        ...
        public List<string[]> GetOccupiedDates(Int64 roomID)
        {
            ...
            {
                var bookings = QueryExecutor($"SELECT dateTo, dateFrom, isRemoved FROM bookings WHERE bookingID = {bookedRooms[i]["bookingID"]}");
                if (bookings == null) return dates;
                if (bookings[0]["isRemoved"].ToString() == "0")
                {
